...
public class ItemGrenadeExtra extends ItemEnumMulti {
	...
	public static Consumer<EntityGrenadeUniversal> EXTRA_PROXY = (grenade) -> {
		if(grenade.getTimer() >= 10 && grenade.getTimer() % 3 == 0) {
			...
			for(EntityLivingBase e : living) {
				...
				if(e instanceof EntityPlayer && !Integrations.canTargetPlayerWGC(grenade.getOwnerParty(), e.getUniqueID(), grenade.worldObj)) continue;
	...
	public static Consumer<EntityGrenadeUniversal> EXTRA_TRIPLEX = (grenade) -> {
		...
		for(int i = 0; i < 3; i++) {
			EntityGrenadeUniversal triplet = new EntityGrenadeUniversal(grenade.worldObj, frag, grenade.getOwnerParty()).setTrail(EntityGrenadeUniversal.TRAIL_TRIPLET);
