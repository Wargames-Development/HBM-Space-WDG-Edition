...
public class ItemGrenadeFilling extends ItemEnumMulti {
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_CLUSTER = (grenade) -> {
		...
		for(int i = 0; i < frags; i++) {
			EntityBulletBaseMK4 bullet = new EntityBulletBaseMK4(grenade.worldObj, pellets, 15F, 0F, grenade.worldObj.rand.nextFloat() * 2F * (float) Math.PI, (grenade.worldObj.rand.nextFloat() * 0.5F + 0.5F) * (float) Math.PI, grenade.getOwnerParty());
			bullet.setPosition(grenade.posX, grenade.posY + 0.05, grenade.posZ);
			bullet.setThrower(grenade.getThrower());
			bullet.motionX *= 0.5;
			bullet.motionY *= 0.75;
			bullet.motionZ *= 0.5;
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_CLUSTER_HEAVY = (grenade) -> {
		...
		int frags = 15;
		for(int i = 0; i < frags; i++) {
			EntityBulletBaseMK4 bullet = new EntityBulletBaseMK4(grenade.worldObj, pellets_heavy, 30F, 0F, grenade.worldObj.rand.nextFloat() * 2F * (float) Math.PI, (grenade.worldObj.rand.nextFloat() * 0.5F + 0.5F) * (float) Math.PI, grenade.getOwnerParty());
			bullet.setPosition(grenade.posX, grenade.posY + 0.05, grenade.posZ);
			bullet.setThrower(grenade.getThrower());
			bullet.motionX *= 0.5;
			bullet.motionY *= 1.25;
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_DEMO = (grenade) -> {
		ExplosionVNT vnt = new ExplosionVNT(grenade.worldObj, grenade.posX, grenade.posY, grenade.posZ, 5F, grenade.getOwnerParty(), grenade.getThrower());
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_INC = (grenade) -> {
		...
		EntityFireLingering fire = new EntityFireLingering(world).setArea(6, 2).setDuration(200).setType(EntityFireLingering.TYPE_DIESEL).setOwnerParty(grenade.getOwnerParty());
		...
		for(int dx = -2; dx <= 2; dx++) for(int dy = -2; dy <= 2; dy++) for(int dz = -2; dz <= 2; dz++) {
			...
			if(!Integrations.canExplodeBlockWGC(grenade.getOwnerParty(), world, x, z)) continue;
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_WP = (grenade) -> {
		World world = grenade.worldObj;
		standardExplode(grenade, 3F, 10F);
		EntityFireLingering fire = new EntityFireLingering(world).setArea(6, 2).setDuration(600).setType(EntityFireLingering.TYPE_PHOSPHORUS).setOwnerParty(grenade.getOwnerParty());
		fire.setPosition(grenade.posX, grenade.posY, grenade.posZ);
		world.spawnEntityInWorld(fire);
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_EMP = (grenade) -> {
		...
		ExplosionNukeGeneric.empBlast(grenade.getOwnerParty(), grenade.worldObj, (int) Math.floor(grenade.posX), (int) Math.floor(grenade.posY), (int) Math.floor(grenade.posZ), 5);
	};
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_LASER = (grenade) -> { // yeah this is good, we like this one
		...
		for(EntityLivingBase target : potentialTargets) {
			...
			if(target instanceof EntityPlayer && !Integrations.canTargetPlayerWGC(grenade.getOwnerParty(), target.getUniqueID(), grenade.worldObj)) continue;
			...
			EntityBulletBeamBase sub = new EntityBulletBeamBase(grenade.worldObj, laser, 30).setOwnerParty(grenade.getOwnerParty());
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_NUKE = (grenade) -> {
		ExplosionVNT vnt = new ExplosionVNT(grenade.worldObj, grenade.posX, grenade.posY, grenade.posZ, 10, grenade.getOwnerParty(), grenade.getThrower());
		vnt.setEntityProcessor(new EntityProcessorCrossSmooth(2, 100).withRangeMod(1.5F));
		vnt.setPlayerProcessor(new PlayerProcessorStandard());
		vnt.explode();

		incrementRad(grenade.getOwnerParty(), grenade.worldObj, grenade.posX, grenade.posY, grenade.posZ, 1F);
		spawnMush(grenade);
	};
	...
	public static Consumer<EntityGrenadeUniversal> EXPLODE_NUKE_DEMO = (grenade) -> {
		...
		incrementRad(grenade.getOwnerParty(), grenade.worldObj, grenade.posX, grenade.posY, grenade.posZ, 1.5F);
		spawnMush(grenade);
	};

	public static Consumer<EntityGrenadeUniversal> EXPLODE_SCHRAB = (grenade) -> {
		EntityNukeExplosionMK3 ex = EntityNukeExplosionMK3.statFacFleija(grenade.getOwnerParty(), grenade.worldObj, grenade.posX, grenade.posY, grenade.posZ, 20);
		if(!ex.isDead) {
	...
	public static void incrementRad(UUID ownerParty, World world, double posX, double posY, double posZ, float mult) {
		for(int i = -2; i <= 2; i++) { for(int j = -2; j <= 2; j++) {
				if(Math.abs(i) + Math.abs(j) < 4) {
					int blockX = (int) Math.floor(posX + i * 16);
					int blockZ = (int) Math.floor(posZ + j * 16);
					if(!Integrations.canIrradiateWGC(ownerParty, world, blockX >> 4, blockZ >> 4)) continue;
					ChunkRadiationManager.proxy.incrementRad(world, blockX, (int) Math.floor(posY), blockZ, 50F / (Math.abs(i) + Math.abs(j) + 1) * mult);
				}
			}
		}
	}
	...
	public static void explodeStandardEnergy(EntityGrenadeUniversal grenade, float damage, float range, DamageClass damageClass, float r, float g, float b, float scale) {
		ExplosionVNT vnt = new ExplosionVNT(grenade.worldObj, grenade.posX, grenade.posY, grenade.posZ, range, grenade.getOwnerParty(), grenade.getThrower());
	...
	public static void standardFragmentation(EntityGrenadeUniversal grenade, float frags) {
		...
		for(int i = 0; i < frags; i++) {
			EntityBulletBaseMK4 bullet = new EntityBulletBaseMK4(grenade.worldObj, fragmentation, 10F, 0F, grenade.worldObj.rand.nextFloat() * 2F * (float) Math.PI, (grenade.worldObj.rand.nextFloat() - 0.5F) * 2F * (float) Math.PI, grenade.getOwnerParty());
			bullet.setPosition(grenade.posX, grenade.posY + 0.05, grenade.posZ);
			bullet.setThrower(grenade.getThrower());
			grenade.worldObj.spawnEntityInWorld(bullet);
		}
