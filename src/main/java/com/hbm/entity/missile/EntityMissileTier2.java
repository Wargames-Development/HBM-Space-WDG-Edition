...
public abstract class EntityMissileTier2 extends EntityMissileBaseNT {
	...
	public static class EntityMissileIncendiaryStrong extends EntityMissileTier2 {
		...
		@Override public void onMissileImpact(MovingObjectPosition mop) {
			...
			ExplosionChaos.igniteFlammableBlocks(ownerParty,this.worldObj, (int)((float)this.posX + 0.5F), (int)((float)this.posY + 0.5F), (int)((float)this.posZ + 0.5F), 25);
		}
	...
	public static class EntityMissileClusterStrong extends EntityMissileTier2 {
		...
		@Override public void onMissileImpact(MovingObjectPosition mop) {
			...
			if(mop == null) {
				ExplosionChaos.cluster(ownerParty, this.worldObj, this.posX, this.posY, this.posZ, 50, this.rotationYaw, this.rotationPitch, (float) Math.PI * 0.25F, (float) Math.PI * 0.25F, 1F);
				this.killMissile();
			}
			else{
			this.explodeStandard(15F, 30, true);
			ExplosionChaos.cluster(ownerParty, this.worldObj, this.posX, this.posY, this.posZ, 50, this.rotationYaw, this.rotationPitch, (float) Math.PI * 0.25F, (float) Math.PI * 0.25F, 1F);
			}
		}
