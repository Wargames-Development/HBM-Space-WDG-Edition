...
public class EntityGrenadeUniversal extends EntityThrowableInterp {
	...
	public EntityGrenadeUniversal(World world, ItemStack grenade) {
		this(world, grenade, null);
	}

	public EntityGrenadeUniversal(World world, ItemStack grenade, UUID ownerParty) {
		super(world);
	...
	public EntityGrenadeUniversal(World world, EntityPlayer thrower, ItemStack grenade) {
		...
		this.ownerParty = thrower != null ? thrower.getUniqueID() : null;
		ItemStack copy = grenade.copy();
		copy.stackSize = 1;
		this.dataWatcher.updateObject(DW_GRENADE, copy);
	...
	public UUID getOwnerParty() {
		if(ownerParty == null && getThrower() != null) ownerParty = getThrower().getUniqueID();
		return ownerParty;
	}

	public EntityGrenadeUniversal setOwnerParty(UUID ownerParty) {
		this.ownerParty = ownerParty;
		return this;
	}
	...
	public void explode() {
		...
		// Effects are authoritative on the server. The client only removes its local entity.
		if(worldObj.isRemote) return;

		int x = (int) Math.floor(posX);
		int y = (int) Math.floor(posY);
		int z = (int) Math.floor(posZ);
		if(!Integrations.canDetonateWGC(getOwnerParty(), worldObj, x, y, z)) return;
		...
		if(GeneralConfig.enableExtendedLogging) {
			String s = "null";
			if(getThrower() instanceof EntityPlayer) s = ((EntityPlayer) getThrower()).getDisplayName();
			MainRegistry.logger.log(Level.INFO, "[GREN] Set off grenade at " + x + " / " + y + " / " + z + " by " + s + "!");
		}
	}
