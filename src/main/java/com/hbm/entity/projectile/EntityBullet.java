...
public class EntityBullet extends Entity implements IProjectile {
	...
	private void captureOwner(Entity entity) {
		if(entity instanceof EntityPlayer) this.ownerParty = entity.getUniqueID();
	}

	public UUID getOwnerParty() {
		if(this.ownerParty == null && this.shootingEntity instanceof EntityPlayer) this.ownerParty = this.shootingEntity.getUniqueID();
	...
	// WDG compatibility overloads retained for callers created before the upstream bullet deprecation.
	public EntityBullet(UUID ownerParty, World world, EntityLivingBase shooter, EntityLivingBase target, float velocity, float inaccuracy) {
		this(world, shooter, target, velocity, inaccuracy);
		this.ownerParty = ownerParty != null ? ownerParty : this.ownerParty;
	}

	public EntityBullet(UUID ownerParty, World world, EntityLivingBase shooter, float velocity, int dmgMin, int dmgMax, boolean instakill, boolean rad) {
		this(world, shooter, velocity, dmgMin, dmgMax, instakill, rad);
		this.ownerParty = ownerParty != null ? ownerParty : this.ownerParty;
	}

	public EntityBullet(UUID ownerParty, World world, EntityLivingBase shooter, float velocity, int dmgMin, int dmgMax, boolean instakill, String type) {
		this(world, shooter, velocity, dmgMin, dmgMax, instakill, type);
		this.ownerParty = ownerParty != null ? ownerParty : this.ownerParty;
	}
	...
	public void onUpdate() {
		...
		if (this.inGround && !this.getIsCritical()) {
			...
			for (i = 0; i < list.size(); ++i) {
				...
				if(entity1 instanceof EntityPlayer && !Integrations.canHarmPlayerWGC(this.getOwnerParty(), entity1.getUniqueID(), this.worldObj)) continue;
	...
	public void writeEntityToNBT(NBTTagCompound p_70014_1_) {
		...
		if(this.getOwnerParty() != null) p_70014_1_.setString("ownerParty", this.getOwnerParty().toString());
	}
	...
	public void readEntityFromNBT(NBTTagCompound p_70037_1_) {
		...
		if(p_70037_1_.hasKey("ownerParty")) {
			try { this.ownerParty = UUID.fromString(p_70037_1_.getString("ownerParty")); }
