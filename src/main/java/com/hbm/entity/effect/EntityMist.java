...
public class EntityMist extends Entity {
	...
	public void onEntityUpdate() {
		...
		if(!worldObj.isRemote) {
			...
			if(type.hasTrait(FT_VentRadiation.class)) {
				...
				if (Integrations.canIrradiateWGC(party, worldObj, ((int) Math.floor(posX))>> 4, ((int) Math.floor(posZ) >> 4))) {
			...
			if(type.hasTrait(FT_Flammable.class) && this.isBurning()) {
				if(Integrations.canDetonateWGC(party,worldObj,(int)Math.floor(posX),(int)Math.floor(posY),(int)Math.floor(posZ))) {
	...
	protected void affect(Entity e, double intensity) {
		...
		UUID victim = e instanceof EntityPlayer ? ((EntityPlayer) e).getUniqueID() : null;

		if(victim == null || Integrations.canHarmPlayerWGC(party,victim,worldObj)) {
		...
		if(type.hasTrait(Fluids.DELICIOUS.getClass())) {//Not harmful, unchecked
			if(living != null && living.isEntityAlive()) {
				living.heal(2F * (float) intensity);
			}
		}

		if(this.isExtinguishing(type)) {//Not harmful, unchecked
			e.extinguish();
		}
