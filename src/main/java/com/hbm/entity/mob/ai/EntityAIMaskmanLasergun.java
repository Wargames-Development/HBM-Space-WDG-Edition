...
public class EntityAIMaskmanLasergun extends EntityAIBase {
    ...
    public void updateTask() {
		...
		if(timer <= 0) {
			...
			switch(attack) {
			case ORB:
				EntityBulletBaseNT orb = new EntityBulletBaseNT(null,owner.worldObj, BulletConfigSyncingUtil.MASKMAN_ORB, owner, target, 2.0F, 0);
				orb.motionY += 0.5D;
			...
			case MISSILE:
				EntityBulletBaseNT missile = new EntityBulletBaseNT(null,owner.worldObj, BulletConfigSyncingUtil.MASKMAN_ROCKET, owner, target, 1.0F, 0);
			...
			case SPLASH:
				...
				for(int i = 0; i < 5; i++) {
					EntityBulletBaseNT tracer = new EntityBulletBaseNT(null,owner.worldObj, BulletConfigSyncingUtil.MASKMAN_TRACER, owner, target, 1.0F, 0.05F);
